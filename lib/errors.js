...
var ChildrenMismatchedError = function (result, longError) {
    var message = result.errors
        ...
        return "Child not matched " + index + ":\n" + diffString(error.actual, error.expected, longError);
    })
        .join('\n\n');
...
var diffString = function (actual, expected, longError) {
    var actualString = typeof actual === 'string'
        ? actual
        : JSON.stringify(removeGrandchildren(actual), null, 2);
    var expectedString = typeof expected === 'string'
        ? expected
        : wildcard_1.isWildcard(expected)
            ? 'WILDCARD'
