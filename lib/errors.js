...
var ChildrenMismatchedError = function (result, longError) {
    var message = result.errors
        .map(function (error, index) { return "Error" + index + ": " + diffString(error.actual, error.expected, longError); }).join('\n');
...
var diffString = function (actual, expected, longError) {
    var actualString = typeof actual === 'string' ? actual : JSON.stringify(removeGrandchildren(actual), null, 2);
    var expectedString = typeof expected === 'string' ? expected : wildcard_1.isWildcard(expected)
        ? 'WILDCARD'
...
var removeGrandchildren = function (vnode) { return (__assign(__assign({}, vnode), { children: vnode.children
        ? vnode.children
            .map(function (c) {
            return typeof c === 'object' ? __assign(__assign({}, c), { children: '...' }) : c;
        })
